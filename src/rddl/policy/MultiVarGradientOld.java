...
public class MultiVarGradientOld extends Policy {
	
	public MultiVarGradientOld () { 
		super();
	}
	
	public MultiVarGradientOld(String instance_name) {
		super(instance_name);
	}
	...
	//oldQ * this = ConvergeT
	final double RelativeErr = 0.001;
		...
		for (; h < maxDepth; h++) {
			...
			if(SearchDepth == -1){
				if(ifReverseAccu){
					...
					estimateAvgIte += t11 - t00;
					
					long timeLeft = (long) (_timeAllowed - (System.currentTimeMillis() - t0));
					int estLayer = h < maxVarDepth ? h : maxVarDepth;
					if (estimateAvgIte * numOfIte / actionSeen > timeLeft) {
						System.out.println("Estimate time: "
								+ estimateAvgIte + "/" + actionSeen + ", and we will have "
								+ numOfIte
	...
	public ArrayList<Double> UpdateAllwtProj(State s, TreeExp F) throws EvalException{
		...
		if(currentRound < 5){
			...
			if(randomRestart != 0){
				...
				_visCounter.SeenInTotal += triedAct.size();
		...
		if(randomRestart != 0){
			...
			System.out.println("Seen: " + triedAct.size());
		}
		else{
	...
	public ArrayList<PVAR_INST_DEF> getActions(State s) throws EvalException {
		...
		triedAct = new ArrayList<ArrayList<Double>>();
