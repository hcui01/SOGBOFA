...
public class ForwardAcc extends Policy {
	
	public ForwardAcc () { 
		super();
	}
	
	public ForwardAcc(String instance_name) {
		super(instance_name);
	}
	...
	TreeExp BuildF(State s) throws EvalException{
		...
		for (; h < maxDepth; h++) {
			...
			if(SearchDepth == -1 && countActBits >= 3){
	...
	public ArrayList<Double> UpdateAllwtProj(State s, TreeExp F) throws EvalException{
		...
		while(!stopAlgorithm){
			...
			while(!ifConverge && !stopAlgorithm){
				...
				if(ifReverseAccu){
					...
					F.RevAccGradient(que, delta, actionProb);
		...
		if(currentRound < 5 && randomRestart == 0){
			_visCounter.randomTime --;
		}
		if(currentRound < 5){
			Records record = new Records();
			record.fileAppend("visCounterEveryStep", _visCounter.randomInTotal + " " + _visCounter.randomTime 
					+ " " + _visCounter.randomInTotal/_visCounter.randomTime);
		}
		...
		for(ArrayList<Integer>a: triedAct){
			System.out.println(a);
		}
