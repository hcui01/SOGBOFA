...
public class FixStepGradient extends Policy {
	
	public FixStepGradient () { 
		super();
	}
	
	public FixStepGradient(String instance_name) {
		super(instance_name);
	}
	...
	double ConvergeT = 0.0000000000000000000000000001;
	...
	//oldQ * this = ConvergeT
	final double RelativeErr = 0.00000001;
	...
	TreeExp BuildF(State s) throws EvalException{
		...
		for (; h < maxDepth; h++) {
			...
			if(countActBits >= 3){
				...
				HashMap<TExp, Double> gradRec = new HashMap<TExp, Double>();
				HashMap<TExp, Double> valRec = new HashMap<TExp, Double>();
	...
	public double FndAlpha(State s, TreeExp F, ArrayList<Double> actionProb, ArrayList<Double> delta){
		...
		while(true){
			...
			if(realBest == maxAlpha / AlphaTime){
				...
				//System.out.println("Alpha is too large, will try alpha between 0 and " + maxAlpha );
			}
			else{
				break;
			}
	...
	public ArrayList<Double> UpdateAllwtProj(State s, TreeExp F) throws EvalException{
		...
		while(!stopAlgorithm){
			...
			int doDownTimes = 0;
			if(iteCounter == 2){
				//randomly initialize actionProb
				for(int i = 0; i < actionProb.size(); i ++){
