...
public class MultiVarGradientCon extends Policy {
	
	public MultiVarGradientCon () { 
		super();
	}
	
	public MultiVarGradientCon(String instance_name) {
		super(instance_name);
	}
	boolean ifConverge = false;
	final double convergeNorm = -1;
	...
	final double actionEstimateAdj = 2;
	...
	final boolean ifMultiLayer = true;
	...
	//oldQ * this = ConvergeT
	final double RelativeErr = 0.00001;
	...
	final boolean ifPrintEst = true;
	final boolean ifPrintBit = false;
	final boolean ifPrintBest = true;
			...
			if(SearchDepth == -1){
				if(ifReverseAccu){
					...
					if(h <= MaxEstimateDepth){
						...
						staCounter ++;
						actionSeen += UpdateAllwtProjSimple(s, F);;
						/*
						int tryTimes = 10;
						...
						*/
						...
						if(h >= 1){
							...
							seesActionEachStep = actionSeen / staCounter;
							if(ifPrintEst)
							System.out.println("Build Tree: " + thisBuildTreeT + ", Estimate time: "
									+ thisTImeUpdate + "/" + seesActionEachStep + ", and we will have "
									+ numOfIte
							...
							estimateUpdate = Double.valueOf(thisTImeUpdate + estimateDifference) / seesActionEachStep * numOfIte;
	...
	public ArrayList<Double> UpdateAllwtProj(State s, TreeExp F) throws EvalException{
		...
		System.out.println(bestComplete);
	...
	public ArrayList<Double> SampleNumAct(ArrayList<Double> varVal, State s){
		...
		boolean[] mute = new boolean[countActBits* maxVarDepth];
		double[] res = new double[countActBits * maxVarDepth]; 
	...
	public ArrayList<PVAR_INST_DEF> getActions(State s) throws EvalException {
		...
		Double maxTry = numInTotal * 2;
		if(trySeeing > maxTry){
			trySeeing = maxTry.intValue();
		}
		if(numOfIte > numInTotal){
