...
public class NYYR11 extends Policy {
	...
	public NYYR11(String instance_name) {
		super(instance_name);
	}
...
//	}
	...
	public ArrayList<PVAR_INST_DEF> getActions(State s) throws EvalException {
		...
		while(true){
			...
			for( ; h < instance._nHorizon * searchRatio && currentRound + h <= instance._nHorizon; h++ ) {
				...
				if(h == 0){
					...
					try{
						RandomConcurrentPolicyIndex _actionGenerator = new RandomConcurrentPolicyIndex(s);
						_randomAction = _actionGenerator.getAActionDup(cs, numOfSample);
						//
						/*
						_randomAction = new ArrayList<RDDL.PVAR_INST_DEF>();
						for(PVAR_NAME n: s._alActionNames){
							ArrayList<ArrayList<LCONST>> l = s.generateAtoms(n);
							for(ArrayList<LCONST> t: l)
							_randomAction.add(new PVAR_INST_DEF(n._sPVarName, (Object)(1.0/15.0), t));
						}
						*/
		...
		if(currentRound < 5){
			_visCounter.randomInTotal += performanceRecords.size();
		}
//		actions = new RandomConcurrentPolicy().getActions(s);
		...
		System.out.println(performanceRecords.get(bestI).value);
	...
	public boolean IfGenerateALL(State s, INSTANCE instance, DOMAIN domain) throws EvalException{
		...
		double possibleRuns;
		if(t == t0){
			possibleRuns = Double.MAX_VALUE;
		}
		else{
			possibleRuns = Math.ceil(_timeAllowed / (t - t0));
		}
