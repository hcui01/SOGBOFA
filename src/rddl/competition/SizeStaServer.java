...
public class SizeStaServer implements Runnable {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				Runnable runnable = new SizeStaServer(connection, ++ID, rddl, state_viz, port, rdg);
	...
	SizeStaServer (Socket s, int i, RDDL rddl, StateViz state_viz, int port, RandomDataGenerator rgen) {
		this.connection = s;
		this.id = i;
		this.rddl = rddl;
...
//			System.out.println(msg.substring(39));
	...
	static String createXMLSessionInit (int numRounds, double timeAllowed, SizeStaServer server) {
		DocumentBuilderFactory dbf = DocumentBuilderFactory.newInstance();
		try {
			DocumentBuilder db = dbf.newDocumentBuilder();
			Document dom = db.newDocument();
